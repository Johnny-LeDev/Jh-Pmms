----
---- Created Date: [2025-03-02 02:09:10]
---- Author: Johnny
----
---- File: [server.lua]
----
...
AddEventHandler("pmms:start", function(handle, options)
	...
	if restrictedHandles[handle] then
		if restrictedHandles[handle] ~= source then
			errorMessage(source, "Ce lecteur est occupé.")
			return
		end
	...
	if mediaPlayers[handle] then
		...
		elseif (Permissions:check(source)) then
			if (Permissions:check(source) or isUrlAllowed(options.url)) then
				options.title = false
...
AddEventHandler("pmms:loadPermissions", function()
	...
	if (xPlayer) then
		...
		permissions.anyEntity = true
		permissions.customUrl = Permissions:check(source)
		permissions.anyUrl    = Permissions:check(source)
		permissions.manage    = Permissions:check(source)

		TriggerClientEvent("pmms:loadPermissions", source, permissions)
	else
		TriggerClientEvent("pmms:refreshPermissions", source)
	end
end)
...
SetHttpHandler(exports["Jh-Pmms"]:createHttpHandler())
